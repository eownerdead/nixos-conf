...
clangStdenv.mkDerivation rec {
  ...
  mozcdic-ut = fetchurl {
    url = "https://osdn.net/downloads/users/38/38352/mozcdic-ut-${dictdate}.tar.bz2";
    hash = "sha256-Qf5s0YVejhxSOTKhn+MgCuPOXaruTtE5I2k+KH71R+k=";
  };
  ...
  postUnpack = lib.optionalString clangStdenv.isLinux ''
    tar -C $sourceRoot -xf ${mozcdic-ut}
  ...
  postPatch = ''
    ...
    sed -i '/^#.*="true"/ s/^#//' \
      ./mozcdic-ut-${dictdate}/src/make-dictionaries.sh

    cat ./mozcdic-ut-${dictdate}/mozcdic*-ut-*.txt >> \
